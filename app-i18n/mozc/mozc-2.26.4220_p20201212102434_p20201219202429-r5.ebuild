...
if [[ "${PV}" == "9999" ]]; then
	...
	SRC_URI="https://github.com/google/${PN}/archive/${MOZC_GIT_REVISION}.tar.gz -> ${PN}-${PV%%_p*}-${MOZC_DATE}.tar.gz
		...
		ut? ( https://osdn.net/downloads/users/30/30220/mozcdic-ut-${MOZCDIC_UT_REVISION}.tar.bz2 )"
...
src_unpack() {
	if [[ "${PV}" == "9999" ]]; then
		...
		if use ut; then
			unpack mozcdic-ut-${MOZCDIC_UT_REVISION}.tar.bz2
			cat mozcdic-ut-*/mozcdic-*-*.txt >> ${P}/src/data/dictionary_oss/dictionary00.txt || die
		fi
